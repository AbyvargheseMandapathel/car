...
from .models import Car,customusers , Company
...
def add_car(request):
    if request.method == 'POST':
        ...
        new_car = Car.objects.create(company_id_id=company_id, name=name, car_model=car_model, price=price, details=details, image=image)
        new_car.save()
        return redirect(view_car)
    else:
        companies = Company.objects.all()  
        return render(request, 'company/addcar.html', {'companies': companies})
